...
namespace Project2015To2017.Tests
{
    ...
    {
        private List<XElement> ProjectPropertyGroups = new[]
        {
            new XElement("PropertyGroup")
        }.ToList();
        private static AssemblyAttributes BaseAssemblyAttributes() => new AssemblyAttributes
        {
        ...
        public void GenerateAssemblyInfoOnNothingSpecifiedTest()
        {
            ...
            Assert.Equal("GenerateAssemblyInfo", generateAssemblyInfo.Name);
            Assert.Equal("false", generateAssemblyInfo.Value);
            Assert.DoesNotContain(BaseAssemblyAttributes().File, project.Deletions?.ToList());
        }
        ...
        public void MovesAttributesToCsProj()
        {
            ...
            var actualProperties = project.AssemblyAttributeProperties.Select(x => x.ToString()).ToList();
            Assert.Equivalent(expectedProperties, actualProperties);
            ...
            Assert.True(expectedAttributes.Equals(project.AssemblyAttributes));
            Assert.Equal(project.AssemblyAttributeProperties.ToList(), project.PrimaryPropertyGroup().Elements().ToArray());
            Assert.DoesNotContain(BaseAssemblyAttributes().File, project.Deletions?.ToList());
        }
        ...
        public void BlankConfigurationGetsDeleted()
        {
            ...
            Assert.Equivalent(expectedProperties, actualProperties);
            var expectedAttributes = new AssemblyAttributes();
            Assert.Null(project.AssemblyAttributes.Configuration);
            Assert.DoesNotContain(BaseAssemblyAttributes().File, project.Deletions?.ToList());
        }
        ...
        public void EmptyAssemblyInfoIsDeleted()
        {
            ...
            var transform = new AssemblyAttributeTransformation(NoopLogger.Instance);
            transform.Transform(project);
            Assert.Contains(assemblyInfoFile, project.Deletions.ToList());
        }
        ...
        public void RedundantAssemblyInfoIsDeleted()
        {
            ...
            {
                File = assemblyInfoFile,
                FileContents = (CompilationUnitSyntax)CSharpSyntaxTree.ParseText(File.ReadAllText(assemblyInfoFile.FullName)).GetRoot()
            };
            var transform = new AssemblyAttributeTransformation(NoopLogger.Instance);
            transform.Transform(project);
            Assert.Contains(assemblyInfoFile, project.Deletions.ToList());
        ...
        public void ClassInAssemblyInfoIsNotDeleted()
        {
            ...
            var transform = new AssemblyAttributeTransformation(NoopLogger.Instance);
            transform.Transform(project);
            Assert.DoesNotContain(assemblyInfoFile, project.Deletions.ToList());
        }
        ...
        public void GenerateAssemblyInfoFromMultipleFilesTest()
        {
            ...
            var transform = new AssemblyAttributeTransformation(NoopLogger.Instance);
            transform.Transform(project);
            Assert.Equal(0, project.AssemblyAttributeProperties.Count);
            Assert.Equal("false", project.Property("GenerateAssemblyInfo")?.Value);
            Assert.DoesNotContain(BaseAssemblyAttributes().File, project.Deletions?.ToList());
        }
    }
}